...
public static class CalculatorStringParser
{
    ...
    public static double Parse(string str)
    {
        ...
        for (int i = 0; i < func.Length; i++)
        {
            ...
            if (matchFunc.Groups.Count > 1)
            {
                ...
                {
                    case 0:
                        return Parse(left + Math.Sin(Parse(inner)).ToString().Replace(',', '.') + right);

                    case 1:
                        return Parse(left + Math.Cos(Parse(inner)).ToString().Replace(',', '.') + right);

                    case 2:
                        return Parse(left + Math.Tan(Parse(inner)).ToString().Replace(',', '.') + right);

                    case 3:
                        return Parse(left + (1.0 / Math.Tan(Parse(inner))).ToString().Replace(',', '.') + right);
                }
            }
        }
